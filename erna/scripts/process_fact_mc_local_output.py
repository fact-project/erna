...
import erna
from erna import stream_runner_local_output as stream_runner
...
def make_jobs(jar, xml, data_paths, drs_paths,  output_path,
              name, engine, queue, vmem, num_jobs, walltime):
    jobs = []
    # create job objects
...
@click.argument('outfolder', type=click.Path(exists=False, dir_okay=False, file_okay=True, readable=True))
...
@click.option('--name', default="ProcessResult", help='name of the processing', show_default=True)
def main( jar, xml, outfolder, mc_path, queue, walltime, engine, num_jobs, vmem, log_level, port, local, name):
    '''
    Script to execute fact-tools on MonteCarlo files. Use the MC_PATH argument
    to specifiy the folders containing the MC
    '''
    level = logging.INFO
    ...
    job_list = make_jobs(jarpath, xmlpath, mc_paths_array, drs_paths_array, outfolder,
                         name, engine, queue, vmem, num_jobs, walltime)

    job_outputs = gridmap.process_jobs(job_list, max_processes=num_jobs, local=local)
    erna.collect_output(job_outputs, path.join(outfolder, name+".hdf"))
